...
class infomediaWS extends webServiceServer {
  ...
  public function __destruct() {
    ...
    /**
       ...
       @NOTE. This method checks articles on the NEP. 
       Once we get the link from opensearch, we KNOW that it is there .. so
       @TODO handle checkArticle without the nep, or remove method
     **/
    ...
    if (is_array($articles)) {
      foreach ($articles as $article) {
        if ($article->found && $article->links) {  // article might be found in NEP but not on infomedia
          ...
          $response_xmlobj->getReviewResponse->_value = $detail;
      }
    }
...
class FORS {
  ...
  private static function check_in_database($fors, $user, $group) {
    ...
    if ($oci->connect()) {
      ...
      verbose::log(FATAL, "infomediaWS:: check_in_database OCI error: " . $oci->get_error());
      return FALSE;
    }
