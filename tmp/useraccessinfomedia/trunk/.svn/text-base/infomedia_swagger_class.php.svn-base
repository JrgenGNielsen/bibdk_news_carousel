...
class infomedia_webservice {
  ...
  public static function get_article_html($config, $fileId, $try_again=FALSE, $count = 0) {
    ...
    verboseJson::log(DEBUG, 'infomedia_swagger:: GET ARTICLE URL :' . $url);
    $curl = new curl();
    $curl->set_url($url);
...
// TESTING - proxy MUST be set in working copy
    ...
    if($count > 1) {
      verboseJson::log(ERROR, 'infomedia_swagger:: RECURSION BREAK');
      return FALSE;
    }
    ...
    if(!empty($message)){
      verboseJson::log(DEBUG, 'infomedia_swagger:: Message from infomedia : ' . $message);
    }

    $errormessage = '';
    if ($errormessage = helpFunc::check_curl($curl)) {
      verboseJson::log(ERROR, 'infomedia_swagger:: curl status is: ' . $errormessage);
      return false;
    }
 ...
 private static function parse_json($result) {
   ...
   if (isset($json->Message)) {
     if ($json->Message == 'Authorization has been denied for this request.') {
       verboseJson::log(STAT, 'infomedia_swagger::reauthenticate : ' . $json->Message);
       return 'TRY AGAIN';
     }
...
// add logo and diclaimer
  ...
  private static function authenticate($config, $try_again) {
    ...
    if ($token === FALSE || $try_again) {
      ...
      if($token){
        verboseJson::log(DEBUG, 'infomedia_swagger::cache token : ' . $token);
  ...
  private static function check_curl($curl) {
    ...
    if ($status['http_code'] != '200') {
      verboseJson::log(ERROR, 'infomedia_swagger::curl returned: ' . $status['http_code']);
      return FALSE;
    }
  ...
  private static function parse_token($result) {
    ...
    // $result is json
    if ($json === FALSE) {
      verboseJson::log(ERROR, 'infomedia_swagger::could not parse json : ' . $result);
      return FALSE;
    }
    // check for errors
    if (isset($json->error)) {
      verboseJson::log(ERROR, 'infomedia_swagger::error on authenficate : ' . $json->error);
      return FALSE;
    }
